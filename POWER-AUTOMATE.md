...
### «Отправитель исходного письма не совпадает с настройкой CRM»

CRM проверяет `From` внутри переданного EML. Одинаковые адреса в настройках
CRM и в фильтре потока ещё не означают, что повторный тест передаёт письмо этого
отправителя. **Resubmit** использует исходные данные выбранного запуска: старый
тест с Gmail остаётся письмом от Gmail после изменения фильтра на адрес логиста.

1. Откройте нужный запуск в **Run history** и проверьте отправителя в
   **When a new email arrives (V3)**. Выбирайте запуск письма логиста.
2. В **Edit → Export email (V2) → Message Id** должно быть динамическое
   **Message Id** из **When a new email arrives (V3)**, а не вручную вставленный ID.
3. В **HTTP → Body** выберите **Body** именно из **Export email (V2)**.
4. Сохраните поток. Для проверки другого отправителя используйте новое письмо;
   адрес тестового отправителя должен совпадать и в CRM, и в фильтре **From**.
   После теста верните адрес логиста в обе настройки.

Обновлённая `crmApi` сообщает фактический `From` и ожидаемый адрес и сохраняет
время неудачной попытки. CRM подписывает сообщение как результат предыдущей
передачи: сохранение настроек не повторяет импорт и не стирает ошибку. После
успешной передачи ошибка исчезает. Для старых ошибок время не выдумывается.
Если в HTTP передан текст письма вместо EML, сервер отдельно сообщает, что
не нашёл заголовок `From`. Пересылка от себя проверяется по вашему адресу,
а не по упоминанию логиста внутри текста письма.

Для обновления этой диагностики нужны новая версия приложения и публикация
только `crmApi` из актуального репозитория:

```bash
firebase deploy --only functions:crm:crmApi --project logisticsapp-216d5
```

Правила Firestore, ключ Power Automate и данные отправлений менять не требуется.
Поведение повторного запуска: [Microsoft — Troubleshoot a cloud flow](https://learn.microsoft.com/en-us/power-automate/fix-flow-failures).
