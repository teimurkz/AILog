...
export const LeafletRouteMap: React.FC<LeafletRouteMapProps> = ({
  ...
  useEffect(() => {
    ...
    // Determine default asphalt road geometry for the destination
    ...
    // Build polyline route points strictly following paved asphalt highway curves
    ...
    // Build driven trajectory points from historical GPS fixes
    ...
    // 1. Draw Full Planned Highway Route (Dashed Blue Line along real roads)
    if (highwayPoints.length > 1) {
      const highwayPolyline = L.polyline(highwayPoints, {
        color: '#3b82f6',
        weight: 5,
        opacity: 0.7,
        dashArray: '8, 8',
      ...
      // ONLY fit bounds on initial load so user zoom & pan are preserved
      if (isFirstRenderRef.current) {
        const bounds = highwayPolyline.getBounds();
    ...
    // 2. Draw Traveled Path Trajectory (Solid Emerald Green Line along actual driven points)
    if (historyPoints.length > 1) {
      ...
      // Draw small breadcrumb dots along driven trajectory
    ...
    if (waypoints.length > 0) {
      ...
      L.marker([startWp.lat, startWp.lng], { icon: startIcon }).addTo(map);
    }

    // 4. Add Destination Marker (City 🏙️)
    if (waypoints.length > 1) {
      ...
      L.marker([endWp.lat, endWp.lng], { icon: endIcon }).addTo(map);
    }

    // 5. Add Intermediate Waypoint Markers
    ...
    // 6. Neat Compact Small Truck Marker 🚚
    const neatTruckIcon = L.divIcon({
      ...
      html: `
        <div style="position:relative;display:flex;align-items:center;justify-content:center;">
          ...
          <!-- Speed Label Badge -->
          <div style="position:absolute;left:36px;background:rgba(15,23,42,0.9);color:white;padding:3px 7px;border-radius:8px;font-size:10px;font-weight:bold;border:1px solid #10b981;white-space:nowrap;box-shadow:0 4px 10px rgba(0,0,0,0.3);">
            ${speed} км/ч • ${etaFormatted}
          </div>
        </div>
      `,
      iconSize: [140, 36],
    ...
    L.marker([currentLat, currentLng], { icon: neatTruckIcon }).addTo(map);

  }, [currentLat, currentLng, waypoints, speed, etaFormatted, detailedRoadPolyline, locationHistory]);
