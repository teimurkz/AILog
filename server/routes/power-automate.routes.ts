...
// Export email (V2) returns binary content. Microsoft can preserve its original
// media type when forwarding Body, even when HTTP specifies message/rfc822.
// All accepted transports contain the same raw EML, never JSON or HTML Body.
const mailContentTypes = ['message/rfc822', 'application/octet-stream', 'text/plain'];
...
export function createPowerAutomateReceiver(load = getOutlookImportService) {
  ...
  router.use((req, res, next) => {
    ...
    if (!req.is(mailContentTypes)) {
      const receivedType = (req.get('Content-Type') || 'не указан').split(';')[0].trim().slice(0, 100);
      return void res.status(415).json({ error: `Неподдерживаемый Content-Type: ${receivedType}. В HTTP Body выберите Body действия Export email (V2), в Content-Type — message/rfc822. Также поддерживаются application/octet-stream и text/plain с полным письмом EML.` });
    }
  ...
  router.use(raw({ type: mailContentTypes, limit: maxMailBytes }));
